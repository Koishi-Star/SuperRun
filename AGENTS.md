...
- `src/cli.ts` loads `.env`, supports single-turn prompt mode and interactive multi-turn chat mode, exposes `--mode <default|strict>` and `--approvals <ask|allow-all|crazy_auto|reject>`, renders TTY sessions through the current Ink-based interactive shell, and wires local slash commands for settings, history browsing, filtered session browsing, session management, runtime mode switching, command approval switching, command-panel `/duration` tuning, inline/external system prompt editing, `/new [title]`, plus local `@file` and slash-command suggestions while typing.
- `src/agent/loop.ts` manages session state, message history, system prompt assembly, context-budget-aware truncation, per-turn model calls, and a mode-aware tool-calling loop.
...
- `src/config/provider-secrets.ts` stores provider API keys in the app config directory using local obfuscation so interactive users do not need to re-enter them every run.
...
- `src/llm/provider.ts` defines provider profiles, runtime config resolution, API-key source handling, and provider defaults such as Kimi endpoints and models.
- `src/llm/provider-catalog.ts` loads and summarizes Kimi model catalogs, falls back to a local context registry when needed, and attaches catalog metadata to the active provider runtime.
...
- `src/tools/run_command.ts` implements the default-mode command tool with workspace scoping, timeout/output bounds, command classification, interactive approval handoff, pre/post hook integration, and incremental command-output turn events for the TTY renderer.
...
- `src/ui/composer-state.ts` centralizes the shared composer state machine used by the Ink renderer and related input/suggestion logic, including `@file` and slash-command suggestions.
...
- Done: single-turn prompts, interactive multi-turn chat, streaming responses, centralized system prompt assembly, persistent system prompt settings, context-budget-aware history truncation, multi-session persistence across runs, active session restore, session rename, session switching by id/index/title, richer `/sessions` previews, saved history viewing, `/sessions [query]` filtering, Ink-owned `/sessions` `/mode` `/approvals` overlay pickers, read-only `/history` viewer overlays, process-level agent modes (`default` and opt-in `strict`), command approval modes (`ask`, `allow-all`, `crazy_auto`, `reject`), guarded `run_command` in default mode, compound-command risk aggregation in `command_policy.ts`, command-panel minimum dwell tuning via `/duration`, strict-mode specialized read tools, explicit file reading/writing/editing tools, delete-area-backed file removal and restoration, local `/trash` commands, diff-based workspace edit approval, read-only diff review for auto-approved edits, approval/session event persistence in `/history`, provider `reasoning_content` passthrough across tool rounds, Moonshot-compatible tool result serialization, local `@file` validation/suggestions, slash-command suggestions, a semantic Ink input layer, pre/post command hook execution via `command_hooks.ts`, cross-platform shell abstraction, external editor support for system prompt editing, a lightweight rich-text terminal renderer for assistant/history/info/overlay output, non-interactive ANSI rich-text output, lightweight progressive assistant rendering in TTY mode, live command execution result boxes, TTY turn-card rendering with inline approval/review, a single active command-output pane with spinner-backed running states, anti-jitter single-line composer/status treatment, a compact multi-panel welcome/status card, Kimi model catalog refresh and `/model` picker flows, provider-level context budgeting with a fixed TTY context meter, locally persisted provider API keys, and `/new [title]` session creation.
- Not done yet: stronger long-term command policy and trust controls beyond the current compound-command slice, prompt/version handling for evolving system prompts, richer TTY session actions beyond switching/history viewing, multi-provider routing, CLI/module cleanup as `src/cli.ts` grows, a deeper visual system pass for the Ink shell, and higher-fidelity real TTY acceptance checks beyond targeted spot verification.
- Current maturity: the chat loop is now a usable early coding agent with explicit read/write/delete tooling, diff-based edit review, session-level auditability, and a much more coherent TTY interaction model, including live prompt suggestions and bounded TTY viewers, but long-term trust modeling, session-action UX, and broader orchestration cleanup are still intentionally unfinished.
...
1. Build out richer TTY session actions directly inside the Ink shell, especially in `/sessions`, so switch/rename/delete/new flows stop depending on text-only follow-up commands.
2. Extract the session-command and picker orchestration that has accumulated in `src/cli.ts` into smaller modules while preserving the current end-to-end behavior.
3. Continue hardening default-mode command policy beyond the current compound-command slice with clearer trust boundaries, better surfaced denials, and a more deliberate long-term trust model for shell execution versus workspace edits.
4. Improve prompt/version handling so built-in system-prompt evolution does not create confusion for older saved sessions.
5. Add multi-provider routing only after the current tool and session UX surfaces are stable enough to preserve behavior across providers.
